...
class Subsquare(object):
    ...
    def __init__(self, square, sub_x, sub_y):
        self.square = square
        self.world = square.world
        self.col = square.col * 3 + sub_x + 1
        self.row = square.row * 3 + sub_y + 1
        width = self.square.world.subsquare_width
        self.sub_x = sub_x
        self.sub_y = sub_y
        self.name = square.name + "S_N"[sub_y + 1] + "W_E"[sub_x + 1]
        self.id = square.world.get_next_id()
        self.xmin = square.xmin + (self.sub_x + 1) * width
        self.xmax = self.xmin + width
        self.ymin = square.ymin + (self.sub_y + 1) * width
        self.ymax = self.ymin + width
        assert (self.xmin / width) * width == self.xmin
        assert (self.ymin / width) * width == self.ymin
        assert ((self.xmin - 1) / width) * width != self.xmin - 1
    ...
    def set_neighbours(self):
        ...
        for dc in (-1, 0, 1):
            for dr in (-1, 0, 1):
                if (dc, dr) != (0, 0):
                    s = self.world.subgrid.get((self.col + dc,
                                                self.row + dr))
                    if s is not None:
                        result.append(s)
        self.neighbours = result
